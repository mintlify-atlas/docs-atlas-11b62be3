...
description: How to contribute to the Zed code editor project
---

# Contributing to Zed

Thank you for helping us make Zed better! This guide will help you get started with contributing to the Zed codebase.
...
Zed is a large project with many priorities. We spend most of our time working on what we believe the product needs, but we also love working with the community to improve the product in ways we haven't thought of (or had time to get to yet!).

We particularly welcome PRs that are:
...
- Include **tests**. For UI changes, consider updating visual regression tests (see [Building Zed for macOS](/contributing/building-macos#visual-regression-tests))
...
Ready to start contributing? Check out our platform-specific build guides:

- [Building Zed for macOS](/contributing/building-macos)
- [Building Zed for Linux](/contributing/building-linux)
- [Building Zed for Windows](/contributing/building-windows)

Then familiarize yourself with the [Zed architecture](/contributing/architecture) to understand how the codebase is organized.
